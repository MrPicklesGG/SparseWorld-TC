...
class WaymoOccDataset(CustomWaymoDataset):
    ...
    def prepare_train_data(self, index):
        ...
        for fut_num in [1, 2, 3]:
            i_list.append(index + 10*fut_num)
        ...
        if self.filter_empty_gt and (data_queue[0] is None):
            return None
        if self.withimage:
            return self.union2one(data_queue)  # hist + curr frame + fut_gts
        else:
            return data_queue[7]  # curr frame only

    def union2one(self, queue):
        """
        convert sample queue into one single sample.
        Args: 
            queue (List[Dict]): the sample queue, hist[0-6], cur[7], fut[8-10]
        Returns:
        ...
        #         3. add scene_token, occ2img and fut2cur
        scene_token = queue[7]['img_metas'].data['sample_idx'] // 1000
        imgs_list = [each['img'].data for each in queue]
        occ_list = [each['voxel_semantics'] for each in queue]
        mask_list = [each['mask_camera'] for each in queue]
        occ2img_list = []
        fut2cur_list = []
        ...
        for i, each in enumerate(queue):
            ...
            cur_scene_token = metas_map[i]['sample_idx'] // 1000
            metas_map[i]['scene_token']= cur_scene_token
            if cur_scene_token != scene_token:
                if i > 7:
                    metas_map[i] = copy.deepcopy(metas_map[i-1])
                    imgs_list[i] = copy.deepcopy(imgs_list[i-1])
                    occ_list[i] = copy.deepcopy(occ_list[i-1])
                    mask_list[i] = copy.deepcopy(mask_list[i-1])
                else:
                    metas_map[i] = copy.deepcopy(queue[7]['img_metas'].data)
                    imgs_list[i] = copy.deepcopy(imgs_list[7])

        cur_pose = metas_map[7]['ego2global'].astype(np.float32)
        for i in range(8):
            hist_pose = metas_map[i]['ego2global'].astype(np.float32)
            hist_occ2img = [each.astype(np.float32) for each in metas_map[i]['lidar2img']]
            cur_ego2hist_ego = np.linalg.inv(hist_pose) @ cur_pose
            occ2img_list.append(hist_occ2img @ cur_ego2hist_ego)
        for i in range(8, 11):
            fut_pose = metas_map[i]['ego2global'].astype(np.float32)
            fut2cur_list.append(np.linalg.inv(cur_pose) @ fut_pose)
        # Step 2: pack them together
        queue[-1]['img'] = DC(torch.stack(imgs_list[:8]).reshape(-1, 3, 256, 704), cpu_only=False, stack=True)
        queue[-1]['voxel_semantics'] = occ_list[7:]
        queue[-1]['mask_camera'] = mask_list[7:]
        queue[-1]['img_metas'] = DC(metas_map, cpu_only=True)
        queue[-1]['occ2img'] = occ2img_list
        queue[-1]['fut2cur'] = fut2cur_list
        queue = queue[-1]
