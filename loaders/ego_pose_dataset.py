...
class EgoPoseDataset(Dataset):
    def __init__(self, data_infos, target_ref_indices=[0, 2, 4, 6]):
        super(EgoPoseDataset, self).__init__()

        self.data_infos = data_infos
        self.scene_frames = {}
        self.target_ref_indices = target_ref_indices
    ...
    def __getitem__(self, idx):
        ...
        # self.target_ref_indices = [0, 2, 4, 6]

        all_results = []

        for target_ref_offset in self.target_ref_indices:

            target_ref_index = ref_index + target_ref_offset
            if target_ref_index >= len(scene_frame):
                target_ref_index = len(scene_frame) - 1

            target_info = scene_frame[target_ref_index]
            ref_sample_token = target_info['token']
            ref_lidar_from_global = self.get_global_pose(target_info, inverse=True)
            ref_ego_from_lidar = self.get_ego_from_lidar(target_info)

            # NOTE: getting output frames
            output_origin_list = []
            for curr_index in range(len(scene_frame)):
                # if this exists a valid target
                if curr_index == target_ref_index:
            ...
            all_results.append((ref_sample_token, output_origin_tensor, target_ref_offset))
