...
num_layers = 6
num_query = 600
num_frames = 8  # 7 history frames (including sweeps, within past 2s) + 1 current frame
# num_frames = 1  # do not use history frames
num_levels = 4
num_points = 4
num_refines = [1, 4, 16, 32, 64, 128]

future_frames = [0, 2, 4, 6]  # current + future 3s
...
train_pipeline = [
    ...
    dict(type='LoadOccFromFile', occ_root=occ_root, future_frames=future_frames), 
    ...
    dict(type='Collect3D', keys=['img', 'voxel_semantics', 'mask_camera', 'fut2cur', 'fut_list'], meta_keys=(
        'filename', 'ori_shape', 'img_shape', 'pad_shape', 'lidar2img', 'img_timestamp', 'ego2lidar', 'lidar2cam', 'intrinsics', 'extrinsics'))
]
...
test_pipeline = [
    ...
    dict(type='LoadOccFromFile', occ_root=occ_root, future_frames=future_frames, pred_traj=False),
    dict(
        ...
        transforms=[
            ...
            dict(type='Collect3D', keys=['img', 'fut2cur', 'fut_list'], meta_keys=(
                'filename', 'box_type_3d', 'ori_shape', 'img_shape', 'pad_shape',
                'lidar2img', 'img_timestamp', 'ego2lidar', 'lidar2cam', 'intrinsics', 'extrinsics'))
        ])
]
